...
@bot.on_callback_query(filters.regex('set_keep_alive_days') & admins_on_filter)
async def set_keep_alive_days(_, call):
    await callAnswer(call, '📌 设置保号天数')
    send = await editMessage(call,
                             f"⏰【设置保号天数】\n\n请输入一个数字\n取消点击 /cancel\n\n当前保号天数: {config.keep_alive_days}")
    if send is False:
        return
    txt = await callListen(call, 120, back_set_ikb('set_keep_alive_days'))
    if txt is False:
        return

    elif txt.text == '/cancel':
        await txt.delete()
        await editMessage(call, '__您已经取消输入__ **会话已结束！**', buttons=back_set_ikb('set_keep_alive_days'))
    else:
        await txt.delete()
        try:
            days = int(txt.text)
            ...
            await editMessage(call, f"请输入大于0的整数! 您的输入如下: \n\n`{txt.text}`",
                              buttons=back_set_ikb('set_keep_alive_days'))
        else:
            config.keep_alive_days = days
            save_config()
            await editMessage(call,
                              f"⏰ 【保号天数】\n\n{days}天 **Done!**",
                              buttons=back_config_p_ikb)
            LOGGER.info(f"【admin】：{call.from_user.id} - 更新保号天数为{days}天")
