...
class Embyservice(metaclass=Singleton):
    ...
    async def emby_create(self, name: str, days: int, progress_callback=None) -> Union[Tuple[str, str, datetime], bool]:
        """
        创建 Emby 账户
        :param name: 用户名
        :param days: 有效天数
    ...
    async def get_emby_libs(self) -> Optional[List[str]]:
        """
        获取所有媒体库
        :return: 媒体库名称列表
        """
        try:
            ...
            if result.success and result.data:
                libs = [lib['Name'] for lib in result.data]
