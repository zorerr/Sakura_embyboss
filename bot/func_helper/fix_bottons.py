...
from bot import chanel, main_group, bot_name, extra_emby_libs, tz_id, tz_ad, tz_api, _open, sakura_b, \
    schedall, auto_update, fuxx_pitao, moviepilot, red_envelope, config, game
...
def config_preparation() -> InlineKeyboardMarkup:
    ...
    rob_open = '✅' if game.rob_open else '❎'
    bet_open = '✅' if game.bet_open else '❎'
    keyboard = ikb(
        [[('📄 导出日志', 'log_out'), ('📌 设置探针', 'set_tz')],
         ...
         [(f'{rob_open} 抢劫系统', 'set_rob_open'), (f'{bet_open} 赌局系统', 'set_bet_open')],
         [(f'设置游戏倍率({game.magnification}倍)', 'set_game_magnification')],
...
def sched_buttons():
    ...
    clean_logs = '✅' if schedall.clean_logs else '❎'
    ...
    keyboard.add(InlineButton(f'{dayrank} 播放日榜', f'sched-dayrank'),
                 ...
                 InlineButton(f'{clean_logs} 日志清理', f'sched-clean_logs'),
